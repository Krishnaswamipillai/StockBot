...
def select_dice(score, opponent_score):
    """Return "six-sided" unless the sum of SCORE and OPPONENT_SCORE is a
    multiple of 7, in which case return "four-sided" (Hog wild).
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    1 dice scores 3.5 on average
    2 dice scores 5.8837 on average
    3 dice scores 7.4598 on average
    4 dice scores 8.2712 on average
    5 dice scores 8.4713 on average
    6 dice scores 8.7231 on average
    7 dice scores 8.553 on average
    8 dice scores 8.144 on average
    9 dice scores 7.7869 on average
    10 dice scores 7.1665 on average
    6 dice yields highest result
    """
    "*** YOUR CODE HERE ***"
    greatest = 0
    new = 0
    numdice = 1
    for x in range (1,11):
        new = make_averaged(roll_dice,10000)(x,dice)
        print(new)
        if new>greatest:
            greatest = new
            numdice = x
...
def run_experiments():
    ...
    if False: # Change to True to test bacon_strategy
        print('bacon_strategy win rate:', average_win_rate
              (bacon_strategy))
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    if max(opponent_score%10,opponent_score//10)+ 1 +score==(opponent_score/2):
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    #Adjust number of rolls and bacon margin depending on whether hog wild is in effect
    base = BASELINE_NUM_ROLLS
    bacon = BACON_MARGIN
    if (score+opponent_score)%7==0:
        base = 4
        bacon = 5

    #Take risks if ahead, if behind roll lower
    ...
    freebacon = True
    #Decide whether free bacon is a valid option
    if max(opponent_score%10,opponent_score//10)+ 1 + score==opponent_score*2:
        freebacon = False
    
    #Do we win?
    if max(opponent_score%10,opponent_score//10)+1+score>=100 and freebacon==True:
        return 0
    ...
    #Roll 10 to get 1 point for a good swap
    elif score+1==opponent_score/2 and opponent_score - score > 4:
        return 10
    
    #Make them roll 4
    elif (score+opponent_score+max(opponent_score%10,opponent_score//10)+1)%7==0 and freebacon==True:
        return 0
    
    #Make them roll 4 with a roll of 1 (rewards higher risk rolls)
    elif (score+opponent_score+1)%7==0:
        return base + 2
        
    #Tons of bacon points
    elif max(opponent_score%10,opponent_score//10) + 1>=bacon and freebacon==True:
        return 0

    #Roll low or bacon to avoid bad swap
    elif score + 1 == opponent_score*2:
        if max(opponent_score%10,opponent_score//10) + 1 >= bacon - 2:
            return 0
        return base - 4
